...
categories: self-supervised-learning time-series energy-systems
...
<p style="text-align: center; font-size: 0.95em; color: #555;">
  Code available at <a href="https://github.com/richardbarlian/HMSPC" target="_blank" style="font-family: monospace;">HMSPC GitHub</a>
</p>
...
## 3.3 Input-Conditioned Latent Dynamics
...
Gradients are stopped through $\mu_{\text{obs},t_i}$ to prevent the uncertainty loss from distorting the mean prediction _(see [Appendix 10.1](#stopping-gradients))_.
...
GRU weights with orthogonal initialization<sup>[[6]](#ref-6)</sup>, and biases to zero, consistent across all models. KL annealing is applied to both HMSPC and Latent ODE, warming up linearly from 0 to $\beta_{\text{KL}} = 10^{-3}$ over 15 and 10 epochs respectively. HMSPC additionally warms up $\beta$ from 0 to 0.3 over 15 epochs alongside the uncertainty warmup described in [Section 3.5](#35-training-objective) (for more details on hyperparameters, _see [Appendix 10.2](#hyperparams)_).

All models are evaluated on the same train/validation/test split. To ensure each split contains a representative distribution of battery cycle lives, cells are stratified by cycle life using 20 quantile bins, with each bin split 70/15/15 across train, validation, and test _(see [Appendix 10.3](#stratified-split))_. This prevents the common failure mode where short-life and long-life cells cluster in different splits, which would cause evaluation metrics to reflect distributional shift rather than model quality, causing unreasonable decreases in performance during out-of-sample testing. Cells with missing cycle life labels are assigned to the training set. Each model is evaluated over 5 seeds, with mean and standard deviation reported across seeds.
...
<figcaption style="margin-top: -20px; padding-bottom:20px; font-style:italic">
...
<figure style="text-align:center; padding-bottom:20px ">
  <img src="{{ '/assets/RMSE_bar.png' | relative_url }}" width="700">
  <figcaption>Figure 1: RMSE (mV) across all model configurations averaged over 5 seeds ($\pm$ std). Shaded regions distinguish baselines (left) from HMSPC ablations (right). The dashed line marks HMSPC (full) mean RMSE for reference.</figcaption>
</figure>
...
### 10.1 Stopping gradients through $$\mu_{\text{obs}}$$
...
### 10.5 HMSPC Vector Field

<figure style="text-align:center; padding-bottom:20px ">
  <img
    src="{{ '/assets/vector_field.png' | relative_url }}"
    width="500"
    alt="Vector field"
    style="display: block; margin: 0 auto;"
  >
  <figcaption>Figure A1: Vector field of the learned base drift $f_{\theta_1}(z)$ projected onto the first two latent dimensions ($h_1, h_2$), evaluated at $\tilde{u} = 0$ (zero inputs) for seed 42. Color encodes velocity magnitude $\|dh/dt\|_2$. The attractor near $(-1, -1)$ suggests the model has learned a structured resting state in latent space, with faster dynamics in high-magnitude regions.</figcaption>
</figure>
